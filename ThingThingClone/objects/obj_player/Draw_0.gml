...
switch(weapon){
	...
	case spr_rifles: 
					  ...
					  break;
	case spr_shotguns: 
						  wep_x = lengthdir_x(12,mouse_dir);
						  muzzle_x = lengthdir_x(28,mouse_dir);
						  muzzle_y = lengthdir_y(28,mouse_dir);
					      wep_y = lengthdir_y(12,mouse_dir);
						  hand_x = lengthdir_x(16,mouse_dir);
						  hand_y = lengthdir_y(16,mouse_dir);
						  hand_subimg = 1;
						  hand2_x = lengthdir_x(28,mouse_dir);
...
switch(weapon){
	//Melee damage and Start Attack
	case spr_melee:
	dmg = 5;
	if(input_attack && HP >0 ){
		startMeleeAttack = true;
	}
	break;
			
	case spr_pistols:
	//If we input an attack and the cooldown is 0:
	if(input_attack && can_attack && mag > 0 && alarm[2]==-1 && HP >0){
		var bullet = instance_create_depth(x+muzzle_x,y+wep_y-9,depth,obj_bullet);
			bullet.direction = mouse_dir+irandom_range(-bullet_spread,bullet_spread);
			bullet.image_angle = mouse_dir;
			bullet.owner = id;
			var snd_fire;
			//AnimStage  allows us to animate through code.
			animStage = 1;
			hand_subimg = 0;
			can_attack = false;
			...
			switch(subweapon){
				...
				case 1: gun_cooldown = 10; snd_fire = snd_makarov_fire; dmg=1.5;/*Makarov*/ break;
				case 2: gun_cooldown = 35; snd_fire = snd_deagle_fire; dmg=3;/*Deagle*/ break;
				case 3: gun_cooldown = 16; snd_fire = snd_p239_fire; dmg=2.1;/*P239*/ break;
			}
	...
	//If the rate of fire cooldown has reached 0, enable attack again:
	if(gun_cooldown>0)
	{
		gun_cooldown--;
	}else{
	...
	//If we input an attack and the cooldown is 0:
			...
			//Different subweapons got different rates of fire, damage and sound
			switch(subweapon){
				case 0: gun_cooldown = 10; snd_fire = snd_m4_fire; dmg=2.3; /*M4*/ break;
				case 1: gun_cooldown = 90; snd_fire = snd_m1sniper_fire; dmg=9; /*M1 Sniper*/ break;
				case 2: gun_cooldown = 5; snd_fire = snd_ak47_fire; dmg=1.9;/*AK47*/ break;
				case 3: gun_cooldown = 8; snd_fire = snd_umpt_fire; dmg=1.6;/*UMP-T*/ break;
			}
			audio_play_sound(snd_fire,0,0);
			mag--;
			if(mag <= 0  && ammo[2]>0){
...
draw_sprite_ext(spr_playertorso,torso,x,y,flipped,1,1,c_white,image_alpha);
draw_sprite_ext(spr_playerhead,head,x,y-10,1,flipped,mouse_dir,c_white,image_alpha);
draw_sprite_ext(spr_playerfeet,feet,x+walk_x,y-3+walk_y,flipped,1,0,c_white,image_alpha);
draw_sprite_ext(spr_playerfeet,feet,x+3+walk_x2,y-3+walk_y2,flipped,1,0,c_white,image_alpha);
draw_sprite_ext(weapon,subweapon,x+wep_x,y-8+wep_y,1,flipped,mouse_dir+(wep_rotation*flipped),c_white,image_alpha);
draw_sprite_ext(spr_playerhands,hand_subimg,x+hand_x,y+hand_y,1,flipped,mouse_dir+hand_rotation+(wep_rotation*flipped),c_white,image_alpha);
draw_sprite_ext(spr_playerhands,hand2_subimg,x+hand2_x,y+hand2_y,1,flipped,mouse_dir+hand2_rotation+(wep_rotation*flipped),c_white,image_alpha);
